          ...
          fetch(DOWNLOADBLE_APPS_URL, {
            ...
            .then((userApps) => {
              const appsMapped = userApps.data.map((v) => {
                if (!LocalApps.hasOwnProperty(v.id)) {
                  v.isLocal = false;
                  LocalApps[v.id] = v;
                }
              });
...
export const deleteApp = (app, cb) => (dispatch) => {
  let destination = RNFS.DocumentDirectoryPath + '/apps/' + app.id;
  deleteLocalFolder(
    destination,
    (LocalApps) => {
      deleteLocalFolder(
...
export const cancelAppDownload = (app, cb) => (dispatch) => {
  ...
  let destination =
    app.destination || RNFS.DocumentDirectoryPath + '/apps/' + app.id;
  deleteLocalFolder(
    destination,
    (LocalApps) => {
      deleteLocalFolder(
...
export const registerData = (data, cb) => (dispatch) => {
  AsyncStorage.getItem('USERTOKEN')
    .then((token) => {
      AsyncStorage.getItem('APPDATA').then((userData) => {
...
const updateDownloadStatus = (res, app) => (dispatch) => {
  console.log(app);
  if (res && res.jobId) app.jobId = res.jobId;
...
const unzipApp = (app) => {
  app.indexPath = RNFS.DocumentDirectoryPath + '/apps/' + app.id;
  RNFS.mkdir(app.indexPath, {
    NSURLIsExcludedFromBackupKey: true,
  });
  unzip(app.destination, app.indexPath)
    .then(() => {
      app.indexPath = app.indexPath;
      updateLocalApp(app);
    })
    .catch((err) => {
      console.log(err);
...
const downloadAppFilesHelper = (destination, app, success, fail, dispatch) => {
  destination = destination + '/' + app.id + '.zip';
  RNFS.downloadFile({
    fromUrl: app.zip,
    toFile: destination,
    background: true,
